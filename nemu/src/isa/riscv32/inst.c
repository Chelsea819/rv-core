...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = *csr_reg_match(BITS(imm, 11, 0)), *csr_reg_match(BITS(imm, 11, 0)) |= src1);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = *csr_reg_match(BITS(imm, 11, 0)), *csr_reg_match(BITS(imm, 11, 0)) = src1);
